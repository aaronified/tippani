...
- **1,085 Go test functions and 1,844 frontend tests, across 320 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 186 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 134 frontend
  ```
