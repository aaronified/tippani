...
- **1,633 Go test functions and 4,000 frontend tests, across 617 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 268 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 349 frontend
  ```
