...
- **1,545 Go test functions and 3,652 frontend tests, across 567 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 252 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 315 frontend
  ```
