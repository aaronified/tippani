...
- **1,603 Go test functions and 3,874 frontend tests, across 603 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 265 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 338 frontend
  ```
