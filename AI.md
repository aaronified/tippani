...
- **1,596 Go test functions and 3,826 frontend tests, across 598 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 263 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 335 frontend
  ```
