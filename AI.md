...
- **1,264 Go test functions and 2,153 frontend tests, across 379 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 215 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 164 frontend
  ```
