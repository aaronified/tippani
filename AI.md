...
- **1,556 Go test functions and 3,655 frontend tests, across 570 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 254 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 316 frontend
  ```
