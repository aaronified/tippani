...
- **1,592 Go test functions and 3,816 frontend tests, across 595 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 262 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 333 frontend
  ```
