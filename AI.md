...
- **1,713 Go test functions and 4,299 frontend tests, across 647 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 275 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 372 frontend
  ```
  ...
  A habit that fails that reliably is a guard's job: `test/pure/ai-counts.test.js`
