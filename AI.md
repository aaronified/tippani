...
- **1,570 Go test functions and 3,674 frontend tests, across 576 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 258 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 318 frontend
  ```
