...
- **1,672 Go test functions and 4,097 frontend tests, across 632 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 273 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 359 frontend
  ```
