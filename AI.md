...
- **1,665 Go test functions and 4,088 frontend tests, across 630 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 272 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 358 frontend
  ```
