...
- **1,636 Go test functions and 4,008 frontend tests, across 620 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 269 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 351 frontend
  ```
