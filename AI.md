...
- **1,653 Go test functions and 4,051 frontend tests, across 624 test files** — the
  ...
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 270 Go files
  find ./web/frontend -path '*/node_modules' -prune -o \
       -type f \( -name '*.test.*' -o -name '*.spec.*' \) -print | wc -l # 354 frontend
  ```
