...
- **Sign in with your own identity provider.** Authelia, Authentik, Keycloak and any other OpenID
  Connect provider. Each reader links their own account from Profile, and passwords keep working.
- **Pushover notifications** for the daily review, large imports, long metadata fetches and backups.
- **A gethomepage widget** showing works, quotes, forgotten and mastered, with a read-only key.
- **Anthologies export as EPUB**, and can show what the book or film knows and the faces of the
  people in them. A rule can keep an anthology fed from a search.
- **A quote can be a poem, a song, or from 399 BCE.** Approximate years, BCE dates and "c. 1890"
  all work, and a quote can name who it reaches us through.
- **You choose how much of the original a quote shows**, per language: translation only,
  translation first, original first, or original only.
- **The quiz has three difficulties and Random**, can start new lines as already mastered, goes up
  to twenty cards a day, and forgetting a quote no longer costs the whole climb.
- **"Never asked about"** lists the quotes the quiz has not reached yet.
- **Character and performer pages**, with cast shown as faces, and a character's row saying who
  played them.
- **Fandom and Letterboxd as metadata sources**, a picker to take each field from the source you
  prefer, and a mark beside each field showing what every source has for it.
- **Eleven more fields can be edited over a whole selection**, including translation, date and
  language.
- **Prune** removes saved people and characters nothing points at any more.
- **A quote can be duplicated.**

### Changed

- **A new look.** A navigation rail down the left, panels with depth, eight material sets with real
  textures, four grounds per mode, and a type scale that follows your text-size setting everywhere.
- **The Metadata console.** Works, People, Characters, Languages, Categories and Sources, each its
  own screen with issue pills, a type dropdown, full-height lists with sticky filters, and media
  glyphs on every row. Tags and colour categories live here now.
- **Settings in five sections** (Theme, Language and font, Review, Sections, Server), each with its
  own address, showing how much of it you have changed, with a reset per section.
- **One import drop target** that recognises the file itself, a queue that can act on a whole file
  or work, and progress for large imports.
- **A rebuilt add screen** that asks what you are adding first, with translation, language, sticker
  and tags on every form.
- **People and characters are records**, one per person rather than one per spelling, with
  spellings you can merge and split.
- **Every screen's "?" walks you through that screen.** The global onboarding card is gone.
- **Faster pages.** Boards render only what is on screen, and the app no longer downloads itself all
  at once or re-downloads 751 KB on every visit.
- **Bengali** reviewed throughout, read natively rather than from the English.

### Removed

- The Metadata Overview section. Everything it offered is on the other screens.
- The Google Programmable Search key. Picture search uses the sources that need no key.

### Fixed

- Hundreds of fixes across layout, phones, search, imports, metadata lookups and the quiz. The
  detail file lists every one.
