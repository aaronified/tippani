...
func TestFuzzySearch(t *testing.T) {
	...
	res := decode[searchResults](t, c.mustDo("GET", "/search?q=shawshenk&scope=books", nil, 200))
	...
	res = decode[searchResults](t, c.mustDo("GET", "/search?q=shawshank&scope=books", nil, 200))
	...
	res = decode[searchResults](t, c.mustDo("GET", "/search?q=shaw&scope=books", nil, 200))
	if res.Corrected != "" || len(res.Books) != 1 {
		t.Fatalf("prefix q=shaw should not be corrected: %+v", res)
	}

	// Scope respected: a movie-title typo corrects within movies only.
	res = decode[searchResults](t, c.mustDo("GET", "/search?q=casblanca&scope=movies", nil, 200))
	...
	res = decode[searchResults](t, c.mustDo("GET", "/search?q=xyzzyquux", nil, 200))
	...
	res = decode[searchResults](t, c.mustDo("GET", "/search?q=shawsq&scope=books", nil, 200))
...
func TestFuzzySearchCrossUser(t *testing.T) {
	...
	res := decode[searchResults](t, alice.mustDo("GET", "/search?q=xanthophull&scope=books", nil, 200))
	...
	res = decode[searchResults](t, bob.mustDo("GET", "/search?q=xanthophull&scope=books", nil, 200))
