...
| --- | --- | --- | --- |
| Two columns from | `twoCol = W >= 1180` (`:1611`) | `TWO_COLUMN_QUERY` and `.tp-detail-hero { display: block }` at `min-width: 1180px` (`index.css:5390`) | Match, and the app states the number in both places on purpose — which hero COMPONENT renders cannot be a stylesheet decision, because rendering both and hiding one puts two `<h1>`s in the document |
...
| --- | --- | --- | --- |
| A row of its own, per role | `bookRows` gives Author, Translator, Editor (`:986-992`); `filmRows` gives Director and **Studio** (`:1019-1023`); `showRows` the same pair (`:1044-1047`); the game's is Studio and **Publisher** (`:1170-1175`) | one spec per role in `BOOK_FIELDS` / `MOVIE_FIELDS`, flagged `credit` | **Match on the book, the game and the director half of film and show. Departure, open, on Studio** — and this row said Match over it, which is the same failure as the duplicated prop two rows down. A film's and a show's studio has NOWHERE TO BE STORED: `movies.publisher` is 0042's column and belongs to games, and `movies.director` already carries three different things by medium (0040). A Studio row for film and show is a migration and a fetch mapping, not a spec line, and it is named here rather than drawn as done |
| The game's Publisher is a CREDIT, not a text field | `credit: 'org'` (`:1067`) | was a plain field; now `credit: true` with `personKind: 'publisher'`, so it draws the portrait the other credits draw | Match (built here) — the chip's SHAPE is the open org departure below |
| Translator and Editor share a line | `half: true` on both (`:991-992`) | `half: true` on both | Match (built here) |
| A 30px round portrait beside the value | `chipStyle` (`:661-669`), silhouette where there is no photograph | `.cred-face`, one per name the field holds, silhouette fallback | Match — **and it did not render when this row was first written**: the portrait was passed as a SECOND `display` prop on the same `InlineField`, so JSX kept the later one and no face drew at all, while vitest printed `Duplicate "display" attribute` on every run. This row said Match over a feature that was not on the screen. One `display` now, and `work-details-order.test.jsx` asserts one `.cred-face` per credit name — the only shape that fails when the drawing is dropped rather than restyled |
| An organisation's chip is a 9px square, not a circle | `border-radius:9px` on the org branch of `chipStyle` (`:663`) | always round | **Departure, open** — a game's studio goes in the `director` column (0040) and wears a person's circle. The app has no `org` flag on a credit spec; adding one is a spec change and a second picture source, and it is named here rather than implied. What IS fixed is worse and was hiding under it: the face was being looked up under a constant `personKind: 'director'`, so a studio was asked of a film database. `personKindFor` reads `workKinds.js`, which already maps the column per medium |
