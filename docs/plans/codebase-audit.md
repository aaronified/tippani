...
| The game's Publisher is a CREDIT, not a text field | `credit: 'org'` (`:1174`) | was a plain field; now `credit: true` with `personKind: 'publisher'`, so it draws the portrait the other credits draw | Match (built here) — the chip's SHAPE is the open org departure below |
