...
**Status:** four of §3's items shipped in 1.16.0 — the Filters panel, facet
counts, exact phrases and date ranges. Their decisions are in
[`../PLAN.md`](../PLAN.md) §7. What is below is what is left: "more like this",
neighbouring highlights, cross-work duplicates, and sorting.
...
| **Exact phrases** — `"to be or not to be"` | **Shipped 1.16.0.** `PhraseQuery` / `ColumnPhraseQuery`. An unbalanced quote searches rather than errors. |
| **Date ranges** | **Shipped 1.16.0** as the `added_from` / `added_to` facets. The `2025-01..2025-06` and "last 30 days" SPELLINGS are not built — the range is expressed as chips, not as query grammar. |
...
## 1. "More like this", without an embedding model
...
## 4. Sorting and an in-work filter box
