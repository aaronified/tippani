...
Metadata's Overview section, was written on 23 September, after an entry higher up that still
describes Overview. Before 3.0.0 was tagged, parts of the first run were corrected where an audit
of the last commits found them wrong, and one Fixed heading near the foot that a later edit had
dropped was put back; the rest are kept as they were written. Where two entries disagree, the
later-written one is what 3.0.0 does, which is nearly always the one higher up.
