...
time, but not strictly: the Removed run near the foot was written on 23 September, below a run
from 31 August. Before 3.0.0 was tagged, parts of the first run were corrected where an audit of
the last commits found them wrong; the rest are kept as they were written. Where two entries
disagree, the later-written one is what 3.0.0 does, which is nearly always the one higher up.
...
- **The UI glossary on the project site draws its own fonts, textures and logo.** It is written
  to be opened from the repository, so on the site all 367 of its references to them pointed
  outside it and failed to load, from the day the site began publishing it. The site's build
  copies them in now, and its link check reads CSS and refuses a path that leaves the site.
