...
entry written while 3.0 was being built, unedited, for anyone who wants the detail and the reasons.
...
- **Unlinking single sign-on is refused when the account has no password yet.** An account
  ...
  no way in. Set a password with `tippani user passwd` first.
...
- **A deleted account is opaque in the admin's bin.** It shows the account's name and how many
  ...
  Restoring it gives the library back whole to its owner. Deleting an account asks with a
  note saying so: it waits in the admin's bin, sealed, for as long as the bin keeps anything
  else.
- **A password an admin sets is temporary.** An account made from Profile, or one reset with
  `tippani user add`/`passwd` (after the first account), opens on "Choose your own password"
  and nothing else. The server refuses every other request until the reader picks one, and
  refuses the admin's password chosen again.
- **Restore and factory reset start with a backup you download.** Both open on "Download a
  ...
  minutes. The copy is not kept on the server, so it never replaces the archive a restore reads.
