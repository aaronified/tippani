...
entry written while 3.0 was being built, for anyone who wants the detail and the reasons. It reads
roughly newest first, one run of section headings (Added, Security, Changed, Removed, Fixed) at a
time, but not strictly. The Changed and Fixed run at the foot was written on 2 September, below
runs from late August, and the first entry under the Removed heading above it, which retires
Metadata's Overview section, was written on 23 September, after entries higher up that still
describe Overview. Before 3.0.0 was tagged, parts of the first run were corrected where an audit
of the last commits found them wrong, and a Fixed heading was added near the foot, where a later
Removed heading had cut some thirty fixes off from theirs; the rest are kept as they were
written. Where two entries disagree, the later-written one is what 3.0.0 does, which is nearly
always the one higher up.
...
- **Metadata and Settings sit further in from a phone's edge.** On a phone both screens keep
  ...
  than before, so its text sits as far from the screen's edge as it did; on Metadata it moves in
  with the card. On a desk Metadata's cards hold their content a little further from their
  edges, and Settings' cards sit a notch closer together than Metadata's.
...
### Fixed
- **The UI glossary on the project site draws its own fonts, textures and logo.** It is written
  to be opened from the repository, so on the site every one of its references to them (367
  today) pointed outside it and failed to load, as they had since the site began publishing it
  in August. The site's build copies them in now, and its link check reads CSS and refuses a
  path that leaves the site.
