...
| SQLite | `journal_mode=WAL`, `synchronous=FULL`, `busy_timeout=5000`, `foreign_keys=ON`, `_txlock=immediate`; 4-connection pool. **Superseded as built**, twice, both deliberately: `synchronous=NORMAL` became `FULL` (in WAL, NORMAL only fsyncs at checkpoint, so an unclean stop can tear the WAL), and "single writer connection" became `_txlock=immediate` on a shared pool. The single-writer idea was aimed at concurrent-write `SQLITE_BUSY` failures, but it had the cause wrong: almost every write here reads first, and a `DEFERRED` transaction upgrading a read lock to a write lock is an upgrade SQLite fails instantly rather than waiting on, so `busy_timeout` was never consulted. Taking the write lock at `BEGIN` fixes it for every write path at once and leaves readers concurrent, which an in-process single-writer mutex could not do — and unlike a mutex it also holds against a second process. See `internal/store/store.go` and `TestConcurrentReadThenWriteTransactions` |
