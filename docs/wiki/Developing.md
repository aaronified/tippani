...
| `sandbox-probe.sh` | The journeys job's check that Chrome starts with its sandbox on, and on which one: it tries the namespace sandbox, then the setuid helper, and fails the job if neither is seen to start. `test/pure/sandbox-probe.test.js` runs it against a stub Chrome, one case per outcome. |
