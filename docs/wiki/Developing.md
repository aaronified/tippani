...
stopped existing, if a new package, script or workflow has appeared that this document
...
| `site-links.mjs` | Walks an assembled `_site/` and fails on any local `href` or `src` in its pages' markup, or CSS `url()` in its pages or its stylesheets, that does not resolve inside the site. CI's `roadmap` job also runs it on `scripts/testdata/site-links/`, one site that must pass and one that must fail. |
...
you: configure-pages' `enablement` needs a token other than `GITHUB_TOKEN`. `roadmap-bugs.yml`
asks for the write access it needs in its own `permissions:` block, so the repository's
default workflow permissions can stay read-only.
