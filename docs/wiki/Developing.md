...
| `site-links.mjs` | Walks an assembled `_site/` and fails on any local `href` or `src` in its pages' markup, or CSS `url()` in its pages or its stylesheets, that does not resolve inside the site. |
| `seed-issues.mjs` | Backfills a GitHub issue per roadmap item that predates the automation. |
| `doc-map-check.mjs` | Checks this document against the tree: every path it names must exist, every package, script and workflow must be named somewhere in it, and the CI table must name every job in `ci.yml`. |
