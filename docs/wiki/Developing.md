...
| `site-links.mjs` | Walks an assembled `_site/` and fails on any local `href`, `src` or CSS `url()`, in its pages or its stylesheets, that does not resolve inside the site. |
