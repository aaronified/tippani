...
| `claude-kit-setup.sh` | Puts claude-kit back in a fresh cloud container, for the environment's setup script: the marketplace and the plugin, the digest's three off-thresholds in user settings, the kit's commit guard and the exclude line for visual-verify's working screenshots, and `npm ci` for the two packages the test and capture skills drive. Bash, not Node, because it runs before anything is installed. Needs `aaronified/claude-kit` attached to the session. Idempotent. |
