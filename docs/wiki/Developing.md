...
| `claude-kit-setup-check.sh` | Runs `claude-kit-setup.sh` in a sandbox — its own `HOME`, a real `git init`, the installed kit's own `kit_guard.py`, stubs only for `claude` and `npm` — through the cases its history broke on, and exits 1 if any fails. By hand after changing the setup script; not in CI, which cannot fetch the private kit the guard comes from. |
