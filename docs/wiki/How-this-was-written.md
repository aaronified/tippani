...
- **1,739 Go test functions and 4,639 frontend tests, across 717 test files** — the
  ...
  cd web/frontend && npx vitest run                                      # 4,584 of them
  ...
  find ./web/frontend -path '*/node_modules' -prune -o -type f \
       ...
       -print | wc -l                                                    # 437 frontend
  ```

  **`npm test` NO LONGER RUNS ALL OF THEM, AND THAT IS THE POINT.** 4,584 is what
  ...
  the binary and seeds a library. `npm test` runs two projects — 3,773 tests over 317
