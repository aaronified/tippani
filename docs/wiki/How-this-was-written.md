...
- **1,739 Go test functions and 4,644 frontend tests, across 718 test files** — the
  ...
  cd web/frontend && npx vitest run                                      # 4,589 of them
  cd web/frontend && npm run journeys                                    # + 54 in the browser
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 280 Go files
  find ./web/frontend -path '*/node_modules' -prune -o -type f \
       \( -name '*.test.*' -o -name '*.spec.*' -o -name '*.journey.*' \) \
       -print | wc -l                                                    # 438 frontend
  ```

  **`npm test` NO LONGER RUNS ALL OF THEM, AND THAT IS THE POINT.** 4,589 is what
  ...
  the binary and seeds a library. `npm test` runs two projects — 3,778 tests over 318
