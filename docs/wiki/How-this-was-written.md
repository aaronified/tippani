...
- **1,771 Go test functions and 3,858 frontend tests, across 761 test files** — the
  ...
  cd web/frontend && npx vitest run                                      # 3,858 of them
  cd web/frontend && npm run journeys                                    # + 76 in the browser
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 288 Go files
  find ./web/frontend -path '*/node_modules' -prune -o -type f \
       \( -name '*.test.*' -o -name '*.spec.*' -o -name '*.journey.*' \) \
       -print | wc -l                                                    # 473 frontend
  ```

  **`npm test` NO LONGER RUNS ALL OF THEM, AND THAT IS THE POINT.** 4,744 is what
  ...
  the binary and seeds a library. `npm test` runs two projects — 3,850 tests over 328
  files; `npm run lint:rules` runs the third, 894 assertions over 90 files; and
  `npm run journeys` runs 76 tests over 54 files against a real server in a real
  browser, which is the tier that would have caught the bug all this is named after.
  Those 90 READ THE SOURCE TEXT and assert how it is
  ...
  be entirely broken and all 90 of them still pass — none of them runs it. A
