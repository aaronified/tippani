...
- **1,738 Go test functions and 4,470 frontend tests, across 684 test files** — the
  ...
  cd web/frontend && npx vitest run                                      # 4,445 of them
  cd web/frontend && npm run journeys                                    # + 25 in the browser
  ...
  find ./web/frontend -path '*/node_modules' -prune -o -type f \
       ...
       -print | wc -l                                                    # 405 frontend
  ```

  **`npm test` NO LONGER RUNS ALL OF THEM, AND THAT IS THE POINT.** 4,445 is what
  ...
  the binary and seeds a library. `npm test` runs two projects — 3,644 tests over 309
  files; `npm run lint:rules` runs the third, 801 assertions over 78 files; and
  `npm run journeys` runs 25 tests over 18 files against a real server in a real
