...
- **1,815 Go test functions and 4,794 frontend tests, across 799 test files** — the
  ...
  cd web/frontend && npx vitest run                                      # 4,794 of them
  cd web/frontend && npm run journeys                                    # + 114 in the browser
  find . -name '*_test.go' -not -path './node_modules/*' | wc -l         # 294 Go files
  find ./web/frontend -path '*/node_modules' -prune -o -type f \
       \( -name '*.test.*' -o -name '*.spec.*' -o -name '*.journey.*' \) \
       -print | wc -l                                                    # 505 frontend
  ```

  **`npm test` NO LONGER RUNS ALL OF THEM, AND THAT IS THE POINT.** 4,794 is what
  ...
  the binary and seeds a library. `npm test` runs two projects — 3,878 tests over 332
  ...
  1,738 / 4,455 / 681, from 1,738 / 4,470 / 695, from 1,804 / 4,784 / 792, from 1,804 / 4,787 / 794, from 1,805 / 4,787 / 796, from 1,806 / 4,789 / 796, from 1,806 / 4,793 / 797, and from 1,806 / 4,794 / 797 before the
