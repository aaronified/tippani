...
- **1,744 Go test functions and 4,649 frontend tests, across 723 test files** — the
  ...
  cd web/frontend && npx vitest run                                      # 4,594 of them
  ...
  find ./web/frontend -path '*/node_modules' -prune -o -type f \
       ...
       -print | wc -l                                                    # 441 frontend
  ```

  **`npm test` NO LONGER RUNS ALL OF THEM, AND THAT IS THE POINT.** 4,594 is what
  ...
  the binary and seeds a library. `npm test` runs two projects — 3,783 tests over 319
  files; `npm run lint:rules` runs the third, 811 assertions over 81 files; and
  `npm run journeys` runs 55 tests over 39 files against a real server in a real
