...
- **1,738 Go test functions and 4,496 frontend tests, across 700 test files** — the
  ...
  cd web/frontend && npx vitest run                                      # 4,450 of them
  cd web/frontend && npm run journeys                                    # + 46 in the browser
  ...
  find ./web/frontend -path '*/node_modules' -prune -o -type f \
       ...
       -print | wc -l                                                    # 421 frontend
  ```

  **`npm test` NO LONGER RUNS ALL OF THEM, AND THAT IS THE POINT.** 4,450 is what
  ...
  the binary and seeds a library. `npm test` runs two projects — 3,640 tests over 309
  files; `npm run lint:rules` runs the third, 810 assertions over 81 files; and
  `npm run journeys` runs 46 tests over 31 files against a real server in a real
