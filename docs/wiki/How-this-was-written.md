...
- **1,769 Go test functions and 4,800 frontend tests, across 754 test files** — the
  ...
  cd web/frontend && npx vitest run                                      # 4,800 of them
  ...
  find ./web/frontend -path '*/node_modules' -prune -o -type f \
       ...
       -print | wc -l                                                    # 466 frontend
  ```

  **`npm test` NO LONGER RUNS ALL OF THEM, AND THAT IS THE POINT.** 4,724 is what
  ...
  the binary and seeds a library. `npm test` runs two projects — 3,845 tests over 327
  files; `npm run lint:rules` runs the third, 879 assertions over 89 files; and
  `npm run journeys` runs 76 tests over 50 files against a real server in a real
  browser, which is the tier that would have caught the bug all this is named after.
  Those 89 READ THE SOURCE TEXT and assert how it is
  ...
  be entirely broken and all 89 of them still pass — none of them runs it. A
