...
| `TIP-NOTIFY-001` | A Pushover message was not accepted. | Network, `TIPPANI_OFFLINE` switched on, or an invalid user key / application token (Pushover answers 4xx). | Check the keys in Settings → Server → Notifications and press **Send a test**; the import, fetch or backup that triggered the message completed regardless. |
