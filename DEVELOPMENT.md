...
| `web/frontend/test/rules/` | **Lint, not tests.** Seventy-one files that read the SOURCE TEXT and assert how it is spelled — never truncate a name, spacing is a constant, no emoji glyphs, the typescale. Out of `npm test` and into `npm run lint:rules`, because the app can be entirely broken and every one of them still passes. Deleted one at a time as a journey covers its ground. |
...
The frontend suite runs on **Vitest in three projects**: `pure` for value-in, value-out
logic in the node environment, `dom` for components under jsdom, which is paid for only
where a component is genuinely under test, and `rules` — seventy-one files that read the
source text and assert how it is spelled. Only the first two are tests, and only they run
under `npm test`; `rules` runs as `npm run lint:rules`, in its own CI step, because the
app can be entirely broken and every one of them still passes. A suite let a feature ship
100% dead exactly that way. `TZ` is pinned to UTC because several
