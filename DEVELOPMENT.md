...
| `screenshots/frame-scroll.mjs` | Opens a book's detail in Firefox at 1440×900 and 1440×520 and fails if the locked page clips, if a column cannot scroll, or if a column with room below it wears no edge fade. It exists because **jsdom has no layout** — `scrollHeight` there is a constant 0 — so the whole vitest suite is blind to a screen whose height chain is broken, and one shipped. The short window is part of the check: the fixture's books carry three quotes, which fit whatever the frame does, so at one size a broken stream and a working one report the same number. The stylesheet half of the same guard is `test/pure/screen-scroll-chain.test.js`. Run it with `make frame-scroll`. |
