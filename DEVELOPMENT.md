...
| `screenshots/frame-scroll.mjs` | Measures the work detail in Firefox. At 1440×900 and 1440×520 it fails if the locked page clips, if a column cannot scroll, or if a column with room below it wears no edge fade. Across eight widths from 1179 down to 780 it also fails if the **title's lines do not all start in the same place** — a float cutting into a name leaves it complete, unclipped and in two pieces, which every other guard in the repo passes. It exists because **jsdom has no layout** — `scrollHeight` there is a constant 0 — so the whole vitest suite is blind to a screen whose height chain is broken, and one shipped. The short window is part of the check: the fixture's books carry three quotes, which fit whatever the frame does, so at one size a broken stream and a working one report the same number. The stylesheet half of the same guard is `test/pure/screen-scroll-chain.test.js`. Run it with `make frame-scroll`. |
...
| `screenshots/audit-panel.mjs` | **Presses every control on a panel and reports what each one actually did** — a new dialog, a new panel, a focus move, a scroll, a changed body, or `*** NOTHING HAPPENED ***`. The panel is re-opened before each press so one control cannot poison the next, and destructive verbs are skipped by name after an early run removed a credit and shifted every later index. It exists because "half the buttons do not work" is a report no unit test can answer: a handler can be attached, wired to the right prop and still reach a field inside a closed `<details>`, or an id nothing in the repo carries. Both were found this way. `seed-cast.mjs` beside it gives a work the cast row AND the stored speaker link a live chip needs — both, because `characterImagesFor` draws a chip for any name a line carries and only a name the work's cast knows carries the id the press is gated on, so a fixture with dialogue and no cast rows yields chips that are correctly dead. Through the app's own API rather than by writing SQL: the write paths are part of what is being checked, and a fixture built behind them can be a shape the app cannot produce. Point it at a work: `node audit-panel.mjs`. |
| `screenshots/panel-depth.mjs` | Presses a door that a **panel itself** offers and fails when the second panel is not on screen afterwards, or when the depth is not 1. `usePanelStack().open()` REPLACES rather than deepens, so it walks history back and then pushes — and written as `history.go(-n)` plus `requestAnimationFrame(push)` the push landed before the pop and the stack's own handler truncated away the panel just opened. Every `open()` from inside a panel had it, which is why a surface reached off a card always worked and one a panel offers never did. It lives here rather than in vitest because **jsdom dispatches `popstate` on a schedule that does not lose to a frame callback**: `test/dom/panel-opens-panel.test.jsx` passes against the broken ordering, says so in its header, and pins the replace-not-deepen contract instead. Against the rAF version on a freshly rebuilt binary this probe fails five runs out of five (`left NOTHING open (depth 0)`), which is what makes it the guard. Verify the embed by asset hash before trusting a run either way — a stale one, and an unseeded fixture, have each produced a false reading here. Run it with `make panel-depth`. |
