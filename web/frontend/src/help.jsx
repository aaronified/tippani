...
const SHELL_COMMON = [
  ...
  { term: 'Avatar chip', what: 'Opens your profile directly: photo, display name, password, switching accounts, logging out — and, for an admin, the user list and the recovery tools.' },
  {
    ...
    more:
      'Over QUOTES the three are the colour, the ♥ and the quiz toggle, with tags, one sticker across the lot, moving them to another board, and delete behind the ⋯. Over BOOKS, FILMS AND SHOWS they are fill in the missing metadata, move to a shelf, and the quiz toggle, with delete behind the ⋯. On a phone, a long press is the way in. Pick exactly ONE and the ⋯ also offers Edit — the same form the card’s own ⋯ opens; pick a second and it goes, because editing forty at once is a different act with its own form. While the bar is up a plain click picks a card instead of opening it, and it stays up until you dismiss it with the ✕ or Escape. Taking the last card off does NOT put it away: the bar holds, reading “no books selected” with its actions greyed, so changing your mind about which four costs a tap rather than starting again. Dismissing is what clears every tick on the board. Changing a filter drops whatever left the screen, so the number in the bar is always a number it can act on. Deleting asks you to type what it will do; deleting a book says plainly that its quotes go with it, and the whole lot lands in the bin as one entry with one Undo.',
  },
...
const SHELL_TOUCH = [
  ...
  { term: 'Menu (☰)', icon: <IconMenu />, asset: <Gesture kind="swipe-left" />, what: 'The drawer: every screen, your profile, and the pending-import queue. Its Add and Search are the deliberately context-free pair — they open with nothing pre-filled, whatever page you came from.', more: 'Swipe it left or tap outside to close.' },
...
export const HELP = {
  ...
  movies: {
    ...
    entries: [
      ...
      { term: 'Shelf state', what: 'Watching, paused, abandoned, watched — the coloured bar under each poster. A game is playing rather than watching, and played rather than watched; both words appear in the filter once you have a game.' },
  ...
  'movie-detail': {
    ...
    entries: [
      ...
      { term: 'Voice cast', what: 'Fetched from Wikidata, which is the only free structured source of game voice credits and is thin — of 24 well-known titles checked, ten had a usable cast and several had none at all.', more: 'A game with no credits on file shows an honest blank rather than a failed lookup, and the cast is hand-editable either way, so you can type what you know. Cast photos need no key.' },
      ...
      { term: 'State chip', what: 'The shelf: start watching, pause, abandon, finish — with a ×N re-watch count. A game reads start playing and played, and three can be in progress at once where a film allows two.' },
  ...
  search: {
    ...
    entries: [
      ...
      { term: 'Arriving already narrowed', what: 'Searching from a filtered shelf searches the filtered shelf: the Library’s genre, series, shelf, favourites and wishlist arrive as chips, and from a book’s own page you arrive narrowed to that book.', more: 'Every chip is removable, so narrowing costs nothing — widening is one click. The filter sheet and these chips are the same state, so they cannot disagree.' },
      { term: 'A globe in the lens', what: 'Right-click the search button in the top bar and it stops caring where you are: every search becomes a search of everything, and the magnifier draws a small globe to say so.', more: 'Right-click again to put it back. The drawer’s Search has always been global; this makes the top bar’s behave the same way if that is what you want.' },
  ...
  quotes: {
    ...
    entries: [
      ...
      { term: 'Boards', what: 'This screen lists BOARDS, the way the Library lists books — you open one to read what is on it. They are yours: name them, colour them, describe them, give them a picture, make as many as you like.', more: 'Proverbs, Speeches and Others are simply the three you started with, and nothing in the app treats those names as special, so rename or delete them freely. Capturing a quote while a board is open files it on that board, and Move to board — on a quote’s own ⋯, or over a whole selection — re-files it afterwards without opening the form. A new board also asks what it HOLDS: quotes, or proverbs, which is what puts the language and the translation first on the form. That is a property of the board and not of its name, so renaming a proverb board leaves it one.' },
      { term: 'Starting from one of the three', what: 'New board offers Proverbs, Speeches and Others. Pressing one fills the form in — a name, a colour, and what it holds — and stops there, so it is yours to rename before you create it.', more: 'They stay on offer rather than disappearing once you have one, because a board you renamed is one this app can no longer recognise, and the name box refuses a duplicate either way.' },
      ...
      { term: 'Filters', icon: <IconFilter />, what: 'Colour, favourites, tagged, has-notes, and then a tag, a speaker or a medium — the last two built from what you have actually saved, not from a fixed vocabulary.', more: 'On a phone they open as a full-screen sheet with a live result count.' },
  ...
  stats: {
    ...
    entries: [
      ...
      { term: 'Counts', what: 'All three kinds of quote, counted separately: annotations are the lines you kept from books, dialogues the ones from films and shows, and quotes the standalone ones that came from no work at all.', more: 'The header total is the three added up.' },
    ],
  },
  staging: {
  ...
  bin: {
    ...
    entries: [
      ...
      { term: 'Kinds', what: 'Once there is more than one kind in the bin, chips appear to show one kind at a time. Like the search scopes they lose their words to the Button labels setting in Appearance.' },
  ...
  settings: {
    title: 'Settings',
    entries: [
      { term: 'Colour categories', asset: <HelpSwatches />, what: 'What the six highlight colours are called and what they look like — they arrive as Fact, Disagreed, Inspirational, Funny and Meta, and all of them are yours to rename.', more: 'Renaming one changes nothing but the words on your screen — the stored value stays yellow, blue, pink or orange, so exports and imports round-trip exactly as before. Hiding one takes it out of the pickers without touching a single quote already wearing it, and the palette deliberately shares no colour with the app’s own accents.' },
      ...
      { term: 'Features', what: 'Which sections of the app you want to see — the Library, the Catalogue, Quotes.', more: 'Turn one off and its tab goes from the strip, the drawer and the phone bar, its tile goes from Home, its chip goes from Search’s scope row, and ＋ stops offering that kind. Nothing else changes: every book, film and quote stays exactly where it is, the review deck still draws on it, and a link or a bookmark still opens it — so turning it back on finds everything where you left it. One section has to stay, because an app with none has no list to stand in and no ＋ that offers anything.' },
      { term: 'Onboarding', what: 'The guided tour of every feature. Start, replay or resume the whole thing, or pick one section and replay only that — the tour opens on that screen and carries on from there.', more: 'It used to list every section on the card itself; a list you cannot press answers "is this covered?", which is not the question anybody arrives here with.' },
      { term: 'Users', what: 'Everyone on this instance, admin only. Add an account, remove one, or hand over admin — deliberately asymmetric: any member can be made an admin, and an admin can step down, and that is all.', more: 'Nobody can take another admin’s rights away, and nobody can delete another admin’s account either, because that would be the same thing with their whole library attached. The last remaining admin cannot step down, so the instance always has one.' },
      { term: 'Metadata sources', what: 'The API keys lookups run on. Each field edits and saves on its own, and a floppy-with-a-tick beside a field means that key is stored — secrets are write-only, so nothing can ever show you one back.', more: 'Press edit and a box appears below the row; save it blank to clear the key. Which services those keys are for, and what the status chips above them mean, is in the info dot on the card’s heading — one dot for the card, rather than one beside each chip. Multi-author credits sit at the bottom of the same card, because a lookup hands back one credit string and that setting decides whether it names one person or two.' },
      { term: 'IGDB client id & secret', what: 'The games pair, and it is a pair — IGDB authenticates through Twitch, so one field on its own cannot look anything up.', more: 'Register an application at dev.twitch.tv/console for the client id, then press “New Secret” on it for the other half. Unlike films there is no shared built-in to fall back on: the credentials are per-application and rate-limited, so a key shipped with the app would be a queue everybody stands in. With neither set the card says nothing, because an instance with no games in it is not misconfigured; with exactly one set it says so, because that fails at the Twitch handshake and reads like a broken lookup rather than a blank field.' },
      { term: 'Type', icon: <IconType />, what: 'A button on the Appearance card, and a pop-up behind it: every face the app uses, each shown doing its own job — the quote face setting a quote, the label face setting a locator.', more: 'Two alternates apiece, bundled with the app and free to use; nothing is fetched from anywhere. Bold, italic, small caps, all caps and lining figures are per role.' },
  ...
  { term: 'In-depth controls', what: 'Which questions each deck may ask, one switch per type per deck, alongside adaptive intervals, whether Practice moves the schedule, the confirm step, and how much a look lengthens a half-life.', more: 'Back to defaults at the bottom puts every one of them back, not most of them. Three things it will not do: the daily quiz never offers a self-marked card, because a score that mixes marked and self-marked answers can be read as neither; a question type it does not recognise is ignored rather than refused, so a backup from a newer version still restores; and no deck can be left with nothing to ask. That last one is stricter than it looks — Who said this? only fits a line of dialogue, so a deck holding only that would have no question for any book you own, and the switch is held with the reason beside it.' },
  ...
  profile: {
    ...
    entries: [
      ...
      { term: 'Switch account', what: 'Sign in as another user on this server. It asks for that account’s password every time — being an admin does not let you in without one — and each account has a fully separate library.', more: 'The form names the account you are leaving, because on a server with several adjacent names “switch” with no subject is a question about something you cannot see.' },
      ...
      { term: 'Users on this server', icon: <IconPlus />, what: 'Admin only, and part of this screen rather than its own: add an account, grant or revoke admin (the last admin cannot be demoted), or delete an account and everything in its library.', more: 'To hand over, grant another user admin first, then revoke your own.' },
  ...
  capture: {
    ...
    entries: [
      ...
      { term: 'Import', icon: <IconUpload />, asset: <HelpImportFlow />, what: 'Markdown and Readest exports, Kindle Bookcision and your Kindle notebook, Goodreads and Hardcover pages, IMDb quote pages. Everything lands in Pending import first.' },
    ],
  },
}
