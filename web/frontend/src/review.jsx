...
function workNoun(card) {
  if (card.kind === 'screen') return t(card.media_type === 'show' ? 'quiz.noun.show.label' : 'quiz.noun.film.label')
  if (card.kind === 'utterance') return t('quiz.noun.occasion.label')
  return t('quiz.noun.book.label')
}
...
function askLine(card) {
  switch (card.direction) {
    case 'source':
      return t('quiz.question.source.stem', { kind: workNoun(card) })
    case 'quote':
    ...
    default:
      ...
      return t('quiz.question.flip.stem')
  }
}
  ...
  return parts.flatMap((part, i) =>
    i === 0
      ...
      : [
          <span
            key={i}
            aria-label={t('quiz.cloze.blank.aria')}
            style={{
    ...
    <div className="flex items-start gap-2">
      ...
      {(clipped || open) && (
        <FieldIconButton
          ...
          ariaLabel={t(open ? 'quiz.option.collapse.aria' : 'quiz.option.expand.aria')}
          aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
          tooltip={t(open ? 'common.action.show-less.label' : 'quiz.option.expand.tip')}
        />
      )}
    </div>
  )
...
function CardTools({ card, onPatch }) {
  ...
  useEffect(() => {
    ...
    json('GET', `${src.list}?id=${card.id}`).then((r) => {
      ...
      if (!found) return setErr(t('error.load.quiz-card'))
      setRow(found)
  ...
  async function save() {
    ...
    setBusy(false)
    if (!r.ok) return setErr(errText(r, t('error.save.quiz-card')))
    ...
    <div className="mt-3" style={{ borderTop: '1px solid var(--line)', paddingTop: 10 }}>
      {!open ? (
        <button type="button" className="tp-link tp-link-icon" onClick={() => setOpen(true)}>
          <IconEdit />
          <span>{t('quiz.card.fix.label')}</span>
        </button>
      ) : row == null && !err ? (
        <MonoLabel style={{ color: 'var(--faint)' }}>{t('common.action.load.busy')}</MonoLabel>
      ) : (
        <div className="space-y-2">
          <Field label={t('common.field.quote.label')} value={quote} onChange={(e) => setQuote(e.target.value)} />
          <Field label={t('common.field.note.label')} value={note} onChange={(e) => setNote(e.target.value)} />
          ...
          <Field
            ...
            value={tags}
            placeholder={t('quiz.card.tags.placeholder')}
...
export function QuizRunner({ mode, cards, allowSkip, startIndex = 0, onIndex, onAnswered, onDone, submitStep = false }) {
  ...
  async function setItAside() {
    ...
    if (!r.ok) {
      setSetAside(false)
      toast(t('error.setaside.generic'))
      return
    }
    toast(t('quiz.leech.aside.label'))
  }
    ...
    <div key={i} className="review-card-body">
      <div className="mb-2 flex items-baseline justify-between gap-3">
        ...
        <span className="mono-label" style={{ letterSpacing: '.06em' }}>
          {t('quiz.progress.label', { done: i + 1, total: cards.length })}
        </span>
      </div>
          ...
          {graded == null ? (
            ...
            <div style={{ borderTop: '1px solid var(--line)', paddingTop: 12 }}>
              ...
              {/* WHICH OF THE TWO RIGHT ANSWERS THIS WAS. A synonym counts and
                  ...
                  moving for reasons the reader cannot see — beside the words
                  they did not quite write is the only place it means anything. */}
              {lastResp?.synonym && (
                <MonoLabel className="mt-1 block" style={{ color: 'var(--faint)' }}>
                  {t('quiz.cloze.synonym.note')}
                </MonoLabel>
              )}
            </div>
          )}
      ...
      {flip && (
        <div className="mt-3">
          {shown ? (
            <>
              ...
              {graded == null ? (
                ...
                <div className="mt-3 flex items-center gap-2">
                  <Tooltip label={t('quiz.grade.forgot.label')} shortcut="grade-forgot" shiftKey={mode === 'practice'}>
                    <button
                      type="button"
                  ...
                  <Tooltip label={t('quiz.grade.got.label')} shortcut="grade-got" shiftKey={mode === 'practice'}>
                    <button
                      type="button"
            ...
            <Tooltip label={t('quiz.flip.reveal.tip')} shortcut="reveal" shiftKey={mode === 'practice'}>
              <button type="button" className="tp-btn tp-btn-primary tactile" onClick={() => setShown(true)}>
                {t('quiz.flip.reveal.label')} <Kbd keys={shortcutFor('reveal', mode === 'practice')} />
              </button>
            </Tooltip>
          )}
        </div>
      ...
      {answered && leech && (
        <div className="mt-3" style={{ borderTop: '1px solid var(--line)', paddingTop: 10 }}>
          {setAside ? (
            <MonoLabel style={{ color: 'var(--faint)' }}>{t('quiz.leech.aside.label')}</MonoLabel>
          ) : (
            <div className="flex flex-wrap items-center gap-2">
              <MonoLabel style={{ color: 'var(--faint)' }}>
                {t('quiz.leech.count.label', {
                  n: lastResp?.lapse_count ?? card.lapse_count,
                  count: lastResp?.lapse_count ?? card.lapse_count,
                })}
              </MonoLabel>
              <button type="button" className="tp-link" style={{ marginLeft: 'auto' }} onClick={keepGoing}>
                {t('quiz.leech.keep.label')}
              </button>
              <button type="button" className="tp-btn tactile" onClick={setItAside}>
                {t('quiz.leech.aside.action.label')}
              </button>
            </div>
          )}
        </div>
      ...
      <div className="review-card-body">
        ...
        {empty && (
          <div className="py-2 text-center">
            <p className="tp-empty">{t('quiz.practice.empty')}</p>
            <button type="button" className="tp-btn tactile mt-3" onClick={onClose}>{t('common.action.close.label')}</button>
          </div>
        )}
        {cards != null && cards.length > 0 && !done && (
          <>
            ...
            <div className="mt-2 text-right">
              <button type="button" className="tp-link" onClick={onClose}>{t('quiz.practice.end.label')}</button>
            </div>
          </>
        )}
        {done && (
          <div className="py-2 text-center">
            ...
            <p className="mono-label mt-1 mb-3" style={{ letterSpacing: '.06em' }}>
              {t('quiz.round.summary.label', { got: tally.got, missed: tally.forgot })}
            </p>
            <div className="flex items-center justify-center gap-2">
              <button type="button" className="tp-btn tactile" onClick={onClose}>{t('common.action.done.label')}</button>
