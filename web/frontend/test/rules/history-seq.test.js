...
// and neither of them is a screen. A push site that forgets `stampSeq` leaves a
...
// a `stampSeq(...)` call, and not so much that a later argument can fake it.
...
describe('the session history', () => {
  ...
  it('stamps a serial on every entry it pushes', () => {
    const bare = pushes.filter((p) => !/stampSeq\s*\(/.test(p.head)).map((p) => p.at)
    expect(
      bare,
      `these pushState calls write an entry with no tpSeq on it, which breaks the\n` +
        ...
        `stampSeq() from history.js:\n  ${bare.join('\n  ')}`,
    ).toEqual([])
  })

  // stampSeq reads the CURRENT entry's serial and adds one. A second definition —
  ...
  it('keeps the serial in one function', () => {
    expect(files.filter((rel) => /function stampSeq\b/.test(text(rel)))).toEqual(['history.js'])
  })
})
