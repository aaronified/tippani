...
//
// Mutation: with `press('Update password')` for the new password deleted, the
// library never opens and "empty notebook" waits in vain.
//
...
it('a reader given a password by the admin must choose their own before the library opens', async () => {
  ...
  await app.press('Update password')
  await app.see('empty notebook, friend-reader')
