...
# typescale.mjs whether anything clips once the type is turned up. Nothing here
# touches a real data dir.
#
# Seeding is not optional the way it is for captures: an empty screen has no long
# names, no two-author credits and no Bengali title, so it cannot clip. Checking one
...
BIND="${TIPPANI_BIND:-127.0.0.1:8124}"
export TIPPANI_BIND="$BIND"
scratch_prefer_archive typescale node typescale.mjs --base-url "http://$BIND" "$@"
...
# has no use for a display either way.
env -u XAUTHORITY -u DISPLAY -u WAYLAND_DISPLAY \
  node typescale.mjs --base-url "http://$BIND" "$@"
